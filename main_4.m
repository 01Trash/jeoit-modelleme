...
for i=1:m;
    ...
    N = a/sqrt(1-e_kare*sin(B*B));

    X = (N+h)*cos(B)*cos(L);
    Y = (N+h)*cos(B)*sin(L);
    Z = ((1-e_kare)*N+h)*sin(B);
...
##    (((dL*cos(B))^2*t*N)/2)+...
##    ((dL*cos(B))^4)*((5-(t^2)+(9*nu)+(4*nu^2))/24)+...
##    ((dL*cos(B))^6)*((61-(58*t^2)+(t^4)+(270*nu)-(330*t^2*nu))/720)+...
##    ((dL*cos(B))^8)*((1385-(3111*t^2)+(543*t^4)-(t^6))/40320);
##
##    y1 = (N*dL*cos(B))+...
##    ((dL*cos(B))^3)*((1-(t^2)+nu)/6)+...
##    ((dL*cos(B))^5)*((5-(18*t^2)+(t^4)+(14*nu)-(58*t^2*nu))/120)+...
